...
async def process_message(user_message: str, message_history: list, persistent_cart: list) -> dict:
    """
    Process user message using multi-agent system.
    This is a simplified version for the workshop.
    """
    try:
        # For this workshop, we'll route to the Cora (Shopper) agent for simplicity
        # In a full implementation, you would have handoff logic to determine the right agent
        
        # Initialize project client using environment variables
        project_client = AIProjectClient(
        ...
        # Use the Cora/Shopper agent as the main agent
        agent_processor = AgentProcessor(
            project_client=project_client,
            assistant_id=validated_env_vars['cora'],  # Using Cora agent
            agent_type="cora",
            thread_id=thread.id
        )
        ...
        # Process the message
        response_messages = []
        async for message in agent_processor.run_conversation_with_text_stream(user_message):
            response_messages.append(message)
        
        # Combine all response messages
        combined_response = "\n".join(response_messages) if response_messages else "No response received."
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            # Single-agent example - commented out for multi-agent
            # try:
            ...
            # Multi-agent system
            try:
                result = await process_message(user_message, chat_history, persistent_cart)
                await websocket.send_text(fast_json_dumps(result))
            except Exception as e:
                logger.error("Error during multi-agent response generation", exc_info=True)
                await websocket.send_text(fast_json_dumps({"answer": "Error during multi-agent response generation", "error": str(e), "cart": persistent_cart}))
